"""Named Bearer-token authentication for the Apstra MCP server.

Tokens are stored in a JSON file with the following shape::

    {
      "vscode-dev": {
        "token": "apstra_xxxxxxxxxxxxxxxxxxxxxxxxxxx",
...
the companion CLI (``apstra_token_manager.py``) can run on the host without
...
DEFAULT_TOKENS_FILE = os.environ.get("APSTRA_TOKENS_FILE", "/app/secrets/.tokens")

#!/usr/bin/env python3
"""CLI to manage named Bearer tokens for the Apstra MCP server.

Usage examples::

    # Generate a token for a named client (name is REQUIRED)
    python apstra_token_manager.py generate --name "vscode-dev" --description "Laptop VSCode"

    # List existing tokens (values are masked)
    python apstra_token_manager.py list

    # Reveal a token value (recovery)
    python apstra_token_manager.py show --name "vscode-dev"

    # Revoke a token
    python apstra_token_manager.py revoke --name "vscode-dev"

The tokens file location follows the ``APSTRA_TOKENS_FILE`` environment variable
...
def _generate(args: argparse.Namespace) -> int:
    was_empty = len(apstra_auth.load_tokens(args.file)) == 0
    try:
        record = apstra_auth.add_token(args.name, args.description or "", args.file)
    ...
    print("\n⚠️  Save this token securely — it grants access to the Apstra MCP server.")
    if was_empty:
        print(
            "\n🔄 This is the FIRST token. If authentication is enabled "
            "(APSTRA_AUTH_ENABLED=true), the MCP endpoint is currently LOCKED. "
...
def _list(args: argparse.Namespace) -> int:
    tokens = apstra_auth.load_tokens(args.file)
    if not tokens:
...
def _revoke(args: argparse.Namespace) -> int:
    if apstra_auth.revoke_token(args.name, args.file):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Apstra MCP token manager")
    parser.add_argument(
        "--file",
        default=None,
        help=f"Path to the tokens file (default: {apstra_auth.DEFAULT_TOKENS_FILE} or $APSTRA_TOKENS_FILE).",
    )
